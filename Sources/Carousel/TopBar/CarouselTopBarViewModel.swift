...
final class CarouselTopBarViewModel {
    ...
    static func makeState(
        ...
        guard let record else {
            ...
            if store.directoryIsReadable {
                ...
                state.liveness = .noSnapshot
                state.degradedSourceDescription = degradedDescription(dataRoot: store.dataRoot)
            }
            return state
        }
    ...
    static func degradedDescription(dataRoot: CarouselDataRoot) -> String {
        switch dataRoot.origin {
        case .mirror:
            "Mirror of the Hive is unreadable at \(dataRoot.url.path)"
        case .environmentOverride:
            "Injected data root is unreadable at \(dataRoot.url.path)"
        case .settings:
            "Configured data root is unreadable at \(dataRoot.url.path)"
        case .localFallback:
            "Local data root is unreadable at \(dataRoot.url.path)"
        }
    }
}
