    ...
    private static func presentableFailureMessage(from rawMessage: String) -> String {
        ...
        if lowercased.contains("local cmux web server") || lowercased.contains("localhost:") || lowercased.contains("127.0.0.1:") {
            return String(
                ...
                defaultValue: "The local Genesis web server is offline. Start it and retry Open Cloud VM."
            )
        }
        if lowercased.contains("waiting for the cloud vm service")
            ...
            return String(
                ...
                defaultValue: "The Cloud VM service could not create a VM yet. Retry keeps using this pinned Cloud VM slot, and once a VM exists Genesis will always reattach to that same VM."
            )
        }
        if lowercased.contains("password") || lowercased.contains("permission denied") {
            return String(
                localized: "panel.cloudVM.loading.failed.auth",
                defaultValue: "Genesis could not open a passwordless terminal session. Try opening the Cloud VM again."
            )
        }
