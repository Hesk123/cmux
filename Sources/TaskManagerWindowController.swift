        ...
        if processIds.count == 1, let processId = processIds.first {
            ...
            let message = String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "Ask %@ (PID %lld) to terminate gracefully. Genesis will force-kill it if it is still running after a short grace period."
                ),
                row.title,
                Int64(processId)
            )
            ...
            let message = String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "Ask %lld processes to terminate gracefully. Genesis will force-kill remaining processes after a short grace period. PIDs: %@."
                ),
