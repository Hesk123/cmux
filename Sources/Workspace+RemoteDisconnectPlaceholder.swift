...
extension Workspace {
    ...
    nonisolated static func remoteDisconnectPlaceholderScript(
        ...
        let endedLineFormat = String(
            ...
            defaultValue: "[Genesis] remote session disconnected: %s"
        )
        let reconnectLine = String(
            localized: "remote.disconnectBanner.reconnectHint",
            defaultValue: "[Genesis] Press Enter to reconnect. This terminal will stay disconnected until then."
        )
        let reconnectUnavailableLine = String(
            localized: "remote.disconnectBanner.reconnectUnavailableHint",
            defaultValue: "[Genesis] Reconnect this workspace from the sidebar or by running the original Genesis remote command again."
        )
