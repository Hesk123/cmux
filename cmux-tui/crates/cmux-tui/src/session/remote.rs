    ...
    fn apply_tree_delta(
        ...
        let applied = match kind {
            K::WorkspaceAdded | K::WorkspaceClosed | K::WorkspaceRenamed | K::WorkspaceMoved => {
                ...
                let applied = match kind {
                    K::WorkspaceAdded => {
                        ...
                        self.view.workspaces.insert(index, view);
                        if active {
                    ...
                    K::WorkspaceClosed => {
                        match self.view.workspaces.iter().position(|ws| ws.id == workspace_id) {
                            Some(position) => {
                                self.view.workspaces.remove(position);
                    ...
                    K::WorkspaceRenamed => {
                        match self.view.workspaces.iter_mut().find(|ws| ws.id == workspace_id) {
                            Some(workspace) => {
                    ...
                    K::WorkspaceMoved => {
                        match (
                            ...
                            (Some(from), Some(to)) if to < self.view.workspaces.len() => {
                                ...
                                let moved = self.view.workspaces.remove(from);
                                self.view.workspaces.insert(to, moved);
            ...
            K::ScreenAdded | K::ScreenClosed | K::ScreenRenamed => {
                let Some(workspace) =
                    self.view.workspaces.iter_mut().find(|ws| ws.id == workspace_id)
                else {
                    return TreeDeltaApply::Resync;
                };
                match kind {
