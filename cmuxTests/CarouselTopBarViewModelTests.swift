...
struct CarouselTopBarViewModelTests {
    ...
    private struct Harness {
        ...
        init(now: Date = Date(timeIntervalSince1970: 1_788_386_000)) throws {
            ...
            root = CarouselDataRoot(url: directory.url, origin: .environmentOverride)
    ...
    @Test("Row 127: a fresh transcript ending in a user entry means a turn is in flight")
    func freshTranscriptRoles() throws {
        let directory = try TemporaryDirectory()
        let now = Date()
        for (lastRole, expected) in [("user", CarouselAgentActivity.running), ("assistant", .idle)] {
            let transcript = directory.url.appending(path: "\(lastRole).jsonl")
            let lines = #"{"type":"assistant"}"# + "\n" + #"{"type":"system"}"# + "\n" + "{\"type\":\"\(lastRole)\"}\n"
            ...
            #expect(CarouselAgentActivityReader.activity(transcriptPath: transcript.path, now: now) == expected)
        }
    }
